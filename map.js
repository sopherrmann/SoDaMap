...
geocoder.on('result', function(T) {
    ...
    global_result = T;
    let user_pos = add_user_pos_event(currentDate);
});
...
function add_user_pos_event(currentDate) {
    let add_user_pos_event_string = '{\n\t"time_stamp": ' + currentDate + ',\n' +
        ...
        '\t}\n'+
        '}';
    console.log(add_user_pos_event_string);
...
function add_map_interaction_click_event(currentDate, pos, click, bb, Date, clickDate) {
    ...
    let add_map_interaction_click_string = '{ \n \t"time_stamp": ' + currentDate + ',\n' +
        ...
        '}';
    console.log(add_map_interaction_click_string);
...
function add_map_interaction_zoom_event(zoomDate, zoomDateOld, zoomed) {
    let add_map_interaction_zoom_event_string = '{ \n \t"time_stamp": ' + currentDate + ',\n' +
        ...
        '\t}\n' +
        '}';
    console.log(add_map_interaction_zoom_event_string);
...
function add_map_search_event(T, searchText, allSugest, bb, startSearchTime, endSearchTime) {
    ...
    let add_map_search_event_string =  '{ \n \t"starttime_stamp": ' + startSearchTime + ',\n' +
        ...
        '\t\t\t"text_typed":' + searchText +
        '\t\t}\n]\n}';
    console.log(add_map_search_event_string)
}

// Todo: add_spatial-bookmark_event und add_question_event überhaupt rein?

// Todo: Keine Möglichkeiten bestimmte Sachen abzufragen
...
function search() {
    if (value_search === 0 && value_rout === 1) {
        ...
        console.log(add_map_search_event(global_result, searchText, allSugest, bb_search, startSearchTime, endSearchTime));
...
document.getElementById("submitUploadPerson").addEventListener('submit', e => {
    e.preventDefault();
    const information = document.getElementsByClassName('tab1').querySelector("input");
    fetch(serverURL, {
        method: 'POST',
        body: information,
    }).then(response => {
